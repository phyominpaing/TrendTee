...
export const createProduct = asyncHandler(async (req, res) => {
    const { name, description, price, instock_count, category, sizes, colors, images, is_new_arrival, is_feature, rating_count, } = req.body;
    ...
    const uploadedImages = await Promise.all(images.map(async (image) => {
        if (image.file) {
            const uploadedImg = await uploadSingleImage(image.file, "trendtee.com/products");
            return {
...
export const updateProduct = asyncHandler(async (req, res) => {
    ...
    const imagesToDelete = incomingImages
        ? existingProduct.images.filter((image) => {
            return !incomingImages.some((img) => img.public_alt === image.public_alt);
        })
        : [];
    // upload new images
    const uploadedImages = incomingImages
        ? await Promise.all(incomingImages.map(async (image) => {
            if (image.file) {
                const uploadedImg = await uploadSingleImage(image.file, "trendtee.com/products");
                return {
                    url: uploadedImg.image_url,
            ...
            if (!image.url || !image.public_alt) {
                res.status(400);
                throw new Error("Existing product images must include url and public_alt.");
            }
            return {
                url: image.url,
            ...
            await Promise.all(imagesToDelete.map(async (image) => {
                if (image.public_alt) {
                    try {
                        ...
                        console.log(`Error deleting image ${image.public_alt}: ${error}`);
                    }
                }
            }));
        }
...
export const deleteProduct = asyncHandler(async (req, res) => {
    ...
    const imagesToDelete = existingProduct.images.map((image) => image.public_alt);
    try {
